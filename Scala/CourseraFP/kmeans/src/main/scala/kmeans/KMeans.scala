...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val group = points.groupBy(p => findClosest(p, means)) // Group the points by their closest mean
    means.map(mean => (mean, group.getOrElse(mean, GenSeq()))).toMap // Make sure that all the means are in the GenMap, even those with empty GenSeq
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    var converged = true
    var i = 0
    while (converged && i < oldMeans.length && i < newMeans.length) {
      val oldMean = oldMeans(i)
