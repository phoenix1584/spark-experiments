...
package object barneshut {
  ...
  extends Quad {
    ...
    def insert(b: Body): Quad = {
      if (size > minimumSize) {
        ...
        (b +: bodies).foldLeft(Fork(nw, ne, sw, se)) {
          (f, b) => f.insert(b)
        }
      }
      else {
  ...
  class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) {
    ...
    def +=(b: Body): SectorMatrix = {
      def getPos(p: Float, min: Float, max: Float): Int = {
        ((Math.max(Math.min(p, max), min) - min) / sectorSize).toInt
      }

      val x = getPos(b.x, boundaries.minX, boundaries.maxX)
      val y = getPos(b.y, boundaries.minY, boundaries.maxY)
      apply(x, y) += b
