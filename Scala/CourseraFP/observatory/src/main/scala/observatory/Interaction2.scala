...
object Interaction2 {
  ...
  def availableLayers: Seq[Layer] = {
    Seq(
      Layer(Temperatures, temperatureColors, Range(1975, 2015)),
      Layer(Deviations, deviationColors, Range(1990, 2015))
    )
  }
  ...
  def layerUrlPattern(selectedLayer: Signal[Layer], selectedYear: Signal[Year]): Signal[String] = {
    Signal("target/" + selectedLayer().layerName + "/{z}/{x}/{y}{r}.png")
  }
  ...
  def caption(selectedLayer: Signal[Layer], selectedYear: Signal[Year]): Signal[String] = {
    Signal(selectedLayer().layerName + " (" + selectedLayer().toString + ")")
  }
